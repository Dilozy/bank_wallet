...
class ProceedWalletOperationAPI(GenericAPIView):
    ...
    def get_queryset(self):
        """
        Получаем queryset для эндпоинта по динамическим параметрам из URL
        Используем select_for_update для блокировки строки на время транзакции
        """
        return BankWallet.objects.select_for_update().get(id=self.kwargs.get(self.lookup_url_kwarg))
